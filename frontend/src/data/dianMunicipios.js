...
export const findMunicipioByCode = (code = '') =>
  DIAN_MUNICIPIOS.find((item) => item.code === String(code).trim()) || null;

export const findMunicipioByName = (name = '') => {
  const normalized = normalizeText(name);
  if (!normalized) return null;
  return DIAN_MUNICIPIOS.find((item) => normalizeText(item.name) === normalized) || null;
};

export const searchMunicipios = (query = '', limit = 12) => {
  const normalized = normalizeText(query);
  if (!normalized) return DIAN_MUNICIPIOS.slice(0, limit);
  return DIAN_MUNICIPIOS.filter((item) => {
